...
class OffboardControl(Node):
    ...
    def check_isaround_fly(self,target_x_NED,target_y_NED,z):
        ...
        if distance < self.align_maxstep :
            ...
            target_x= current_x + (target_x_NED-current_x)*scale
            target_x= current_y + (target_y_NED-current_y)*scale
            self.fly_to_position(target_x_NED,target_y_NED,z)
    ...
    def adjust_to_target(self):
        ...
        if self.target_position:
            ...
            if not self.first_alignment_checker.first_aligned:
                ...
                self.check_isaround_fly(target_x_NED,target_y_NED,self.takeoff_target_height)  
            ...
            if self.first_alignment_checker.first_aligned and not self.second_alignment_checker.first_aligned:
                ...
                self.check_isaround_fly(target_x_NED2,target_y_NED2,self.takeoff_target_height+self.afterAlign_descentHeight)
                self.second_alignment_check(target_x_NED2, target_y_NED2)
                self.last_found_x_NED = target_x_NED2
                self.last_found_y_NED = target_y_NED2
