...
class MissionState(Enum):
    ...
    # === 阶段 2: 侦察任务 ===
    DROP_COMPLETE_SWITCH_TO_MISSION = 9   # 投水完成，准备切换到航线模式
    TRANSIT_TO_RECON_AREA = 10            # 正在执行航线，飞向侦察区
    RECON_AREA_SWITCH_TO_OFFBOARD = 11    # 到达侦察区，准备切换回Offboard
    ...
    def control_timer_callback(self) -> None:
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            ...
            if self.is_FinishDrop:
                ...
                if self.mission_state == MissionState.RECON_AREA_SWITCH_TO_OFFBOARD:
            ...
            # <<< MODIFIED: 投水完成后，启动切换到航线模式的逻辑 >>>
            # 这个检查现在在所有 Offboard 逻辑的末尾
            if self.is_FinishDrop and self.mission_state != MissionState.MISSION_COMPLETE and self.mission_state.value < MissionState.DROP_COMPLETE_SWITCH_TO_MISSION.value:
                self.get_logger().info("所有载荷投放完毕，准备切换到航线模式，飞往侦察区域...")
                self.start_mission()
                self.mission_state = MissionState.DROP_COMPLETE_SWITCH_TO_MISSION
        
        elif self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_MISSION:
            
            # 状态：DROP_COMPLETE_SWITCH_TO_MISSION -> TRANSIT_TO_RECON_AREA
            if self.mission_state == MissionState.DROP_COMPLETE_SWITCH_TO_MISSION:
                self.get_logger().info("已成功进入航线模式，正在监控飞行距离...")
                self.mission_state = MissionState.TRANSIT_TO_RECON_AREA
            
            # 状态：TRANSIT_TO_RECON_AREA
            elif self.mission_state == MissionState.TRANSIT_TO_RECON_AREA:
                current_dist = math.hypot(self.vehicle_local_position.x - self.initial_x, 
                                          self.vehicle_local_position.y - self.initial_y)
                if self.log_counter % 30 == 0:
                    self.get_logger().info(f"航线飞行中... 距离起飞点: {current_dist:.1f}m / {self.recon_trigger_distance:.1f}m")
                
                # 达到触发距离后，准备切换回Offboard
                if current_dist > self.recon_trigger_distance:
                    self.get_logger().info("已到达侦察区域触发距离，准备切换回Offboard模式。")
                    self.engage_offboard_mode()
                    self.mission_state = MissionState.RECON_AREA_SWITCH_TO_OFFBOARD
                    self.offboard_setpoint_counter = 0

        # --- 场景 3: 意外状态 ---
        # 如果既不是 Offboard 也不是预期的 Mission 模式，则发出警告
        else:
...
def main(args=None) -> None:
    ...
    # === 新增：为侦察任务添加参数 ===
    parser.add_argument('--recon-trigger-distance', type=float, default=8.5,
                        help='从起飞点飞行多远后，触发从航线模式切换回Offboard模式（米）。')
