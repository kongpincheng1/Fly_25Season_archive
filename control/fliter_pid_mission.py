...
class OffboardControl(Node):
    ...
    def __init__(self,args) -> None:
        ...
        # === 抖动抑制 & 派生参数 ===
        self.epsilon_enter = args.pid_enter_threshold  # 原 epsilon
        self.epsilon_exit = args.pid_exit_threshold   # 离开阈值：> enter 才跳回 P
        self.in_pid = False        # PID模式开关
        
        # 视觉测量去毛刺
        self.cam_queue = deque(maxlen=5)  # 最近 5 帧像素
        self.alpha_d = args.d_filter_alpha         # D 项一阶滤波系数
        self.prev_dxdt = 0.0       # 上次 X 方向微分
        self.prev_dydt = 0.0       # 上次 Y 方向微分
        # =========================================
    ...
    def adjust_to_target(self):
        ...
        if self.target_position:
            ...
            # ------ 像素去毛刺 ------
            self.cam_queue.append((dx_cam, dy_cam))
            dx_cam, dy_cam = map(lambda s: sum(s)/len(s), zip(*self.cam_queue))
            
            distance = math.hypot(dx_cam, dy_cam)
            
            # 📌 根据误差大小选择控制策略（带迟滞）
            if distance < self.epsilon_enter:  # 进入 PID
                self.in_pid = True
            elif distance > self.epsilon_exit:  # 退出 PID
                self.in_pid = False
            
            if self.in_pid:
                # ——————— 细调阶段：PID控制 ———————
                if self.log_counter % 10 == 0:
                    self.get_logger().info(f"PID细调阶段 - 误差:{distance:.3f}m < 阈值:{self.epsilon_enter:.3f}m")
                ...
                # 📌 微分项计算（带一阶滤波）
                derivative_x_raw = (error_x - self.last_error_x) / self.dt
                derivative_y_raw = (error_y - self.last_error_y) / self.dt
                
                # 一阶滤波平滑微分项
                derivative_x = self.alpha_d * derivative_x_raw + (1 - self.alpha_d) * self.prev_dxdt
                derivative_y = self.alpha_d * derivative_y_raw + (1 - self.alpha_d) * self.prev_dydt
                
                self.prev_dxdt = derivative_x
                self.prev_dydt = derivative_y
                ...
                # ——————— 大误差阶段：饱和P控制 ———————
                if self.log_counter % 10 == 0:
                    self.get_logger().info(f"饱和P控制阶段 - 误差:{distance:.3f}m >= 阈值:{self.epsilon_exit:.3f}m")
    ...
    def timer_callback(self) -> None:
        ...
        if self.state == MissionState.IDLE:
            ...
            if self.switch_to_offboard_start_time is None:
                self.switch_to_offboard_start_time = self.get_clock().now()

            # 在 PREPARING_OFFBOARD 的循环中检查
            elapsed_time = (self.get_clock().now() - self.switch_to_offboard_start_time).nanoseconds / 1e9
            if elapsed_time > 1.0: # 等待1秒
                self.get_logger().info("设定点指令流已建立...")
            ...
            # 1. 如果计时器还未启动，则启动它
            if self.switch_to_offboard_start_time is None:
                self.get_logger().info("开始尝试切换到Offboard模式，启动5秒超时计时器...")
                self.switch_to_offboard_start_time = self.get_clock().now()
                self.engage_offboard_mode() # 第一次尝试切换
            
            # 2. 检查是否切换成功
            if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
                self.get_logger().info("成功切换到Offboard模式。")
                self.switch_to_offboard_start_time = None # 重置计时器，以便下次使用
                self.DropArea_x = self.vehicle_local_position.x
                self.DropArea_y = self.vehicle_local_position.y
                self.init_yaw = self.vehicle_local_position.heading
                self.state = MissionState.IN_OFFBOARD
                return # 成功了就直接返回，不执行下面的超时检查

            # 3. 如果没成功，检查是否超时
            elapsed_time = (self.get_clock().now() - self.switch_to_offboard_start_time).nanoseconds / 1e9
            
            if elapsed_time > 5.0:
                self.get_logger().error("切换到Offboard模式失败（超时5秒）！任务中止。")
                # 在这里可以决定失败后的行为，例如尝试降落
                self.land()
                self.state = MissionState.TIMEOUT # 进入一个最终的完成/失败状态
                return

            # 4. 如果没超时，每隔一秒重新发送一次切换指令
            if self.log_counter % int(1.0 / self.dt) == 0: # 大约每秒一次
                 self.get_logger().info(f"仍在尝试切换Offboard模式... ({elapsed_time:.1f}s)")
                    ...
                    if visual_state == VisualState.CENTERING:
                        ...
                        if self.Is_Descending_to_depth_camera_height == True :
                            ...
                            if self.Is_Finish_1st_Drop and self.visited_targets_count == 0:                        
                                ...
                                if self.visited_targets_count < 2 and (self.get_clock().now() - self.first_drop_delay).nanoseconds / 1e9 > 1.0:
                                    # 投放完成，不要直接设置下一个目标！
                                    # 而是进入“重新确认”状态
                                    ...
                                    self.get_logger().info("第一次投放完成后等待，wait")
                                    return
                                # 更新任务进度
        ...
        if self.log_counter % 30 == 0: # 每秒打印一次
            self.get_logger().info(f"Callback execution time: {execution_time*1000:.2f} ms")
...
def main(args=None) -> None:
    ...
    # --- PID 行为阈值和限制参数 ---
    parser.add_argument('--pid-enter-threshold', type=float, default=0.20,
                        help='从饱和P控制切换到PID控制的距离阈值 (米)。默认: 0.20.')
    parser.add_argument('--pid-exit-threshold', type=float, default=0.30,
                        help='从PID控制切换回饱和P控制的距离阈值 (米)。默认: 0.30.')
    parser.add_argument('--max-integral', type=float, default=0.2, # 这个值默认等于 align_maxstep
                        help='PID控制器 - 积分项的最大限制值 (防止积分饱和)。默认: 0.2.')
    
    # --- D项滤波器参数 ---
    parser.add_argument('--d-filter-alpha', type=float, default=0.2,
                        help='PID控制器 - D项一阶低通滤波器的alpha系数。默认: 0.2.')
    # --- 选择投放桶 --- 
